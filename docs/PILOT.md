...
- כפתור **סנכרן** מאחד את המאגר עם עותק משותף ב-Google Drive (`StudioMaster/knowledge-base.json`).
  לשיתוף בין כמה אנשים — שתפו את תיקיית `StudioMaster` ב-Drive איתם. גם **הערות העריכה החופשיות**
  שכותבים בסקירת פרק (לשונית "ענן ועריכה") מוזנות להנחיית העורך בעריכה מחדש.
