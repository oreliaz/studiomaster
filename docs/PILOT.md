...
3. תוך כדי: לחץ **Ctrl+Shift+1** (תיקון) / **2** (הדגשה) / **3** (פרק) / **4** (הערה) /
   **5** (פתיח) / **6** (סגיר) —
   כל לחיצה נכנסת למסמך התיקונים ומהבהב חיווי על מסך ה-OBS. בדק אפשר לכתוב הערה קצרה
   לפני הלחיצה והיא נשמרת עם הסימון. בסיום הקלטה הסימונים נשמרים גם ליד קובץ ה-OBS
   (`<capture>.markers.json`) — עבור סוכן האולפן של agentedit.
   בסוף היום: **✅ סיימנו להיום** בדק שולח למפיקה את סיכום היום (דרך סוכן האולפן, `agent.url`,
   ברירת מחדל `http://127.0.0.1:3940`).
...
- **`edit-brief.md`** — מסמך מרוכז: תמלול מלא + טיימקודים + סימונים + חיתוכים + הערות.
  מיועד להזנה ל-Claude / Claude Code **יחד עם קובץ הפרק**, כדי לבקש תיקונים בעריכה או
  חיתוך רילסים נוספים בסשן אחד.
- `edit-log.txt` — יומן הריצה של העורך (לאבחון תקלות).
...
**כל פרק שמצולם באולפן מקבל אוטומטית:** עריכה בסיסית מלאה (`work/final.mp4`), מסמך עבודה
(`edit-brief.md`), חומר הגלם (קובץ ההקלטה + ערוצי האודיו), ועם **"העלה אוטומטית ל-Drive אחרי
עריכה"** (מסומן כברירת מחדל) — הכל עולה ל-Drive מיד בסיום הרינדור, כולל חומר הגלם. יש גם כפתור
**"⬆ העלה את כל מה שטרם עלה"** להעלאת כל הפרקים הישנים בבת אחת.
